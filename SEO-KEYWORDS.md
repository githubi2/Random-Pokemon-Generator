...
- **内容深度批·五页工具块（2026-09-15，待发布）**：wheel / name-gen / whos / mega / nuzlocke 各加 3 个实算深度块（每页 ≥3 个独有块验收）：wheel=spin 数学/过滤池赔率/评级三档；name-gen=七池配额/分片算法实例/词义来源；whos=提示收窄数学/351 来源/池形状；mega=+100 规则与两例外/属性分配/换型换特性；nuzlocke=六队覆盖/掷队机制/共享弱点。数据管线 `E:\pokemen\_blocks_work\stats.json`（全实算）。**同时修正 wheel 页失实文案（8 处）**：原写 "wheel 八段+指针" 与实机（cascade 卡片带 + 金色取景框）不符，改为实机描述（含 FAQ 可见=LD 同步）。- **#10 shiny odds 页**：内容页 + 交互计算器混合体，承接 `pokemon shiny odds` / `shiny odds` / `chance of shiny pokemon` 意图；明确不做 `full odds shiny` 单页（KD 过高）。
...
- **#28 eevee 再补问答（2026-09-21）**：FAQ 9→12 条（How many…／What order…／跨世代条件差异，vis=LD 同步）——依据 Round12–13 顾问共识：第 3 问由 line/tree/chart 调整为条件方向（level 已有 09-18 问答承接）；标题/描述不动；10/06 观察位置与 CTR。**晚间二修（顾问复核）**：新 Q1 删八只名字列表（去与现有第 1 条重复）；What level 问答的 "roughly 220…" 括号句改为 "(the requirement differs by generation — see below)"（220→160 归条件问答独家持有）。
