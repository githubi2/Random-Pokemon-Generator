/* ============================================================
   Pokemon Champions Team Builder — Regulation M-C squads.
   Adapted from the Team Picker engine (same 6-slot core, matchup
   matrix, slot editor, import/export). Champions build changes:
   M-C fixed regulation, formation toggle (singles bring 3 /
   doubles bring 4), M-C item additions + new Mega stones.
...
(function () {
  ...
  var ITEMS = [
    ...
    /* M-C item additions */
    ['leek', 'Leek'], ['binding-band', 'Binding Band'], ['normal-gem', 'Normal Gem'],
    ['terrain-extender', 'Terrain Extender'], ['electric-seed', 'Electric Seed'],
    ['psychic-seed', 'Psychic Seed'], ['misty-seed', 'Misty Seed'], ['grassy-seed', 'Grassy Seed'],
    ['salamencite', 'Salamencite'], ['absolite-z', 'Absolite Z'], ['garchompite-z', 'Garchompite Z'],
    ['lucarionite-z', 'Lucarionite Z'], ['golisopite', 'Golisopite'], ['baxcalibrite', 'Baxcalibrite']
  ];
  var ITEM_MAP = {};
  ...
  var LS_TEAM = 'rpg:champions-team';
  var LS_TEAMS = 'rpg:champions-teams';
  var LS_LIBRARY = 'rpg:champions-library';
  ...
  /* M-C Mega stones: auto-select the matching stone when a Mega is enabled */
  var MEGA_STONES = {
    'salamence-mega': 'salamencite', 'absol-mega-z': 'absolite-z',
    'garchomp-mega-z': 'garchompite-z', 'lucario-mega-z': 'lucarionite-z',
    'golisopod-mega': 'golisopite', 'baxcalibur-mega': 'baxcalibrite'
  };
  ...
  var state = {
    ...
    selSlot: 0,
    filters: {
      reg: 'M-C', types: [], typeMode: 'or', moves: [], moveMode: 'or',
  ...
  function renderSlots() {
    var row = $('tp-slots');
    row.innerHTML = '';
    /* formation: first N filled slots are the battle picks (3 singles / 4 doubles) */
    var battleIdx = {};
    var filledCount = 0;
    state.slots.forEach(function (slot, idx) {
      if (slot && filledCount < state.battleSize) { battleIdx[idx] = true; filledCount++; }
    });
    state.slots.forEach(function (slot, idx) {
      var cell = document.createElement('div');
      cell.className = 'tp-slot' + (idx === state.selSlot ? ' tp-slot-active' : '') + (slot ? ' tp-slot-filled' : '') + (slot && battleIdx[idx] ? ' tp-slot-battle' : '');
      ...
      if (slot) {
        ...
        nm.textContent = slot.nick || displayName(eff ? eff.n : slot.n);
        ...
        if (battleIdx[idx]) {
          ...
          bt.className = 'tp-slot-battle-tag';
          bt.textContent = 'Battle';
          cell.appendChild(bt);
        }
      } else {
  ...
  function updateFilterNote() {
    ...
    var note = bits.length ? 'Showing: ' + bits.join(' · ') : 'Showing the full M-C roster';
  ...
  function renderSlotEditor() {
    ...
    $('slot-edit-name').textContent = d.nick || displayName(d.mega || d.n);
    ...
    if (megaOptions.length && !/mega/.test(eff ? eff.n : '')) {
      ...
      var pickEl = $('slot-edit-mega-pick');
      pickEl.innerHTML = '';
      megaOptions.forEach(function (mo) {
        var o = document.createElement('option');
        o.value = mo.n;
        o.textContent = displayName(mo.n);
        pickEl.appendChild(o);
      });
      if (d.mega) pickEl.value = d.mega;
      pickEl.hidden = megaOptions.length < 2;
    } else {
  ...
  function shareMenu(kind) {
    ...
    else text = 'I built a Pokemon Champions team on Random Pokemon Generator — ' + (state.name || 'My team') + '! Pick yours: ' + buildShareUrl();
  ...
  function init() {
    ...
    /* battle formation toggle (singles: bring 3, doubles: bring 4) */
    (function () {
      var fmtS = $('tp-format-singles');
      var fmtD = $('tp-format-doubles');
      if (!fmtS || !fmtD) return;
      var setFormat = function (size) {
        state.battleSize = size;
        fmtS.classList.toggle('tp-stat-tab-active', size === 3);
        fmtD.classList.toggle('tp-stat-tab-active', size === 4);
        fmtS.setAttribute('aria-selected', size === 3 ? 'true' : 'false');
        fmtD.setAttribute('aria-selected', size === 4 ? 'true' : 'false');
        renderSlots();
      };
      fmtS.addEventListener('click', function () { setFormat(3); });
      fmtD.addEventListener('click', function () { setFormat(4); });
    })();
    ...
    /* regulation is fixed to M-C on this page (no selector) */
    ...
    /* mega toggle live (with form pick + stone auto-set) */
    ...
    megaChk.addEventListener('change', function () {
      ...
      if (megaChk.checked) {
        ...
        var pickEl = $('slot-edit-mega-pick');
        if (pickEl && !pickEl.hidden && pickEl.value) d.mega = pickEl.value;
        if (d.mega && MEGA_STONES[d.mega]) d.item = MEGA_STONES[d.mega];
      } else {
        d.mega = null;
      }
      renderSlotEditor();
    ...
    $('slot-edit-mega-pick').addEventListener('change', function () {
      var ed = state.editing;
      if (!ed) return;
      var d = ed.draft;
      d.mega = this.value;
      if (MEGA_STONES[d.mega]) d.item = MEGA_STONES[d.mega];
      renderSlotEditor();
    });
